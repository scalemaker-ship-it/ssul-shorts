...
## 절대 규칙 — GPT 이미지는 가장 싼 방식으로만 (2026-09-26)

GPT(codex)로 이미지를 만드는 모든 경우 — 이 스킬, 다른 스킬, 모든 프로젝트 스크립트 —
**반드시 `~/.claude/skills/codex-image/scripts/gen.sh` 를 거친다. `codex exec` 를 직접 부르지 않는다.**

- 모델은 측정상 가장 적게 먹는 `gpt-5.6-luna` + `model_reasoning_effort=low` (gen.sh 기본값).
  `CODEX_IMAGE_MODEL` 로 더 비싼 모델을 올리는 건 사용자가 명시적으로 요청했을 때만.
- GPT 는 이미지 생성만 한다. 저장·이동·리사이즈·검증은 gen.sh(sips)와 클로드가 한다.
  코덱스가 셸 명령을 돌리게 두면 턴마다 문맥이 다시 들어가 장당 토큰이 5배 넘게 뛴다.
- 실측(2026-09-26): 기존 방식(기본 gpt-6-astra, 코덱스가 저장까지) 장당 ~13.5만 토큰·80장쯤에서 한도,
  현재 방식 장당 ~2.4만 토큰, 10장에 5시간 한도 2%·주간 1% → 주간 약 1,000장.
- 새 스크립트에서 GPT 이미지가 필요하면 `subprocess.run(["bash", GEN_SH, "--prompt", ..., "--out", ...,
  "--width", W, "--height", H], stdin=DEVNULL)` 식으로 gen.sh 를 호출한다.
- 모델 목록이 바뀌면(`~/.codex/models_cache.json`) fast/efficient 계열로 교체하고 몇 장 뽑아
  `~/.codex/sessions/.../*.jsonl` 의 `rate_limits.used_percent` 로 소모를 재서 가장 싼 것을 고른다.
...
- **N (장수)**: "3장", "여러 개" 등. 기본 1. 20장 이상이면 한도 소모를 한 번 안내하고 확인받는다.
- **비율**: 아래 표로 매핑.
...
## 비용/성능 메모
- 1장 ≈ 24k 토큰(luna, 생성만), 30~60초. N장은 선형 증가. 동시 실행 OK(thread_id 로 결과 분리).
